...
def main():
    ...
    for i in range(len(FILE_LIST)):
        ...
        x_first = ROOT.RooRealVar("xVar", "m(3#mu) [GeV/c^{2}]", 1.91, 2.08); 
        x_first.setBins(n_bins)
        TriHist_first = ROOT.RooDataHist("data", "Data", ROOT.RooArgList(x_first), hist)
        # Parameters of the Gaussian: mean and sigma
        mean1 = ROOT.RooRealVar("mean1", "mean of gaussian1", 1.85, 1.80, 1.90)
        sigma1 = ROOT.RooRealVar("sigma1", "width of gaussian1", 0.025, 0.01, 0.05)
        alpha1 = ROOT.RooRealVar("#alpha1","alpha1",7., 1, 10.)
        nSigma1 = ROOT.RooRealVar("n#sigma1","nSigma1", 1., 0.1, 3.)
        # Parameters of the Gaussian: mean and sigma
        mean2 = ROOT.RooRealVar("mean", "mean of gaussian2", 1.97, 1.93, 2.0)
        sigma2 = ROOT.RooRealVar("sigma", "width of gaussian2", 0.025, 0.01, 0.05)
        alpha2 = ROOT.RooRealVar("#alpha2","alpha2",7., 6, 10.)
        nSigma2 = ROOT.RooRealVar("n#sigma2","nSigma2", 1., 0.1, 3.)
        CB1= ROOT.RooCBShape("CB1", "Crystal Ball PDF 1", x, mean1, sigma1, alpha1, nSigma1)
        CB2= ROOT.RooCBShape("CB2", "Crystal Ball PDF 2", x, mean2, sigma2, alpha2, nSigma2)
        CB2_first= ROOT.RooCBShape("CB2_first", "Crystal Ball PDF 2", x_first, mean2, sigma2, alpha2, nSigma2)
        gamma = ROOT.RooRealVar("#Gamma","Gamma",-1e-1, -2., -1e-2)
        bkgExpPdf = ROOT.RooExponential("bkgExpPdf","bkgExpPdf",x, gamma)
        bkgExpPdf_first = ROOT.RooExponential("bkgExpPdf","bkgExpPdf",x_first, gamma)
        ...
        totalCBExpPDF_first = ROOT.RooAddPdf("totalCBExpPDF_first","totalCBExpPDF_first", ROOT.RooArgList(CB2_first, bkgExpPdf_first), ROOT.RooArgList(nSig2, nBkgExp))
        totalCBExpPDF_first.fitTo(TriHist_first)
        mean2.setConstant(True)
        sigma2.setConstant(True)
        alpha2.setConstant(True)
        nSigma2.setConstant(True)
        sigma1.setVal(sigma2.getVal())
        sigma1.setConstant(True)
        alpha1.setVal(alpha2.getVal())
        alpha1.setConstant(True)
        nSigma1.setVal(nSigma2.getVal())
        nSigma1.setConstant(True)
        totalCBExpPDF = ROOT.RooAddPdf("totalCBExpPDF","totalCBExpPDF", ROOT.RooArgList(CB1, CB2, bkgExpPdf), ROOT.RooArgList(nSig1, nSig2, nBkgExp))
        totalCBExpPDF.fitTo(TriHist)
        xframe = x.frame()
        xframe.SetTitle(hist.GetTitle())
        TriHist.plotOn(xframe)
        totalCBExpPDF.plotOn(xframe, ROOT.RooFit.Components(ROOT.RooArgSet(CB1)), ROOT.RooFit.LineColor(ROOT.kOrange+8), ROOT.RooFit.LineStyle(ROOT.kDashed))
        totalCBExpPDF.plotOn(xframe, ROOT.RooFit.Components(ROOT.RooArgSet(CB2)), ROOT.RooFit.LineColor(ROOT.kOrange+8), ROOT.RooFit.LineStyle(ROOT.kDashed))
        ...
        dicanvas.SaveAs(dir_path + "/"+ FILE_LIST[i]+"_FITCB_preliminary_triplet_mass_plot_pyroot_pull(3).png")
        canvas.Clear()
        dicanvas.Clear()
        ...
        yields.append(Ds_charged_yield)#signal_yield)
        yields_errors.append(Ds_charged_yield_err)#signal_yield_err)
    print(yields)
    print(yields_errors)
    '''
    fig, axs = plt.subplots(1,1, figsize = (7,7))
