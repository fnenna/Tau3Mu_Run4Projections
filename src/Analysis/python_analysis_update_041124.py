...
def DimuonMass(sub_df):
    ...
    invariant_mass = ak.where(sub_df["Mu0102_charge"]!=0, 0, invariant_mass)
    return invariant_mass
    
def ak_abs(array):
    return ak.where(array < 0, -array, array)
...
def twoMuonsFinder(dataframe):
    ...
    #print(muon_pt)
    index_muon = ak.local_index(muon_pt, axis = 1)
    #print(index_muon)
    index_mu01 = ak.local_index(mu01_pt, axis = 1)
    index_mu02 = ak.local_index(mu02_pt, axis = 1)
    pairs_tri_index_1, pairs_index_1= ak.unzip(ak.cartesian([index_mu01, index_muon]))
    pairs_tri_index_2, pairs_index_2 = ak.unzip(ak.cartesian([index_mu02, index_muon]))
    pairs_pt_1 = ak.cartesian([mu01_pt, muon_pt])
    pairs_eta_1 = ak.cartesian([mu01_eta, muon_eta])
    pairs_phi_1 = ak.cartesian([mu01_phi, muon_phi])
    pairs_pt_2 = ak.cartesian([mu02_pt, muon_pt])
    pairs_eta_2 = ak.cartesian([mu02_eta, muon_eta])
    pairs_phi_2 = ak.cartesian([mu02_phi, muon_phi])
...
def trackFinder(dataframe):
    ...
    index_track = ak.local_index(track_pt, axis = 1)
    #print(index_muon)
    index_trk_trip = ak.local_index(trk_trip_pt, axis = 1)
    pairs_pt_tr = ak.cartesian([trk_trip_pt, track_pt])
    pairs_eta_tr = ak.cartesian([trk_trip_eta, track_eta])
    pairs_phi_tr = ak.cartesian([trk_trip_phi, track_phi])
    ...
    _, pairs_index_tr = ak.unzip(ak.cartesian([trk_trip_pt, index_track]))
    pairs_tri_index_tr, _ = ak.unzip(ak.cartesian([index_trk_trip, track_pt]))
...
def contains_elements(array1, array2):
    return ak.Array([[elem in sub2 for elem in sub1] for sub1, sub2 in zip(array1, array2)])


def trigger_match(dataframe):
    HLT_obj_pt = dataframe.MuonPt_HLT2017
    pt1, HLT_obj_pt = ak.unzip(ak.cartesian([dataframe.Mu01_Pt, HLT_obj_pt]))
    index_pt1, _ = ak.unzip(ak.cartesian([ak.local_index(dataframe.Mu01_Pt), HLT_obj_pt]))
    dpt1 = abs(pt1 - HLT_obj_pt)/pt1
    pt2, HLT_obj_pt = ak.unzip(ak.cartesian([dataframe.Mu02_Pt, HLT_obj_pt]))
    index_pt2, _ = ak.unzip(ak.cartesian([ak.local_index(dataframe.Mu02_Pt), HLT_obj_pt]))
    dpt2 = abs(pt2 - HLT_obj_pt)/pt2
    HLT_obj_phi = dataframe.MuonPhi_HLT2017
    phi1, HLT_obj_phi = ak.unzip(ak.cartesian([dataframe.Mu01_Phi, HLT_obj_phi]))
    dphi1 = abs(phi1 - HLT_obj_phi)
    phi2, HLT_obj_phi = ak.unzip(ak.cartesian([dataframe.Mu02_Phi, HLT_obj_phi]))
    dphi2 = abs(phi2 - HLT_obj_phi)
    HLT_obj_eta = dataframe.MuonEta_HLT2017
    eta1, HLT_obj_eta = ak.unzip(ak.cartesian([dataframe.Mu01_Eta, HLT_obj_eta]))
    deta1 = abs(eta1 - HLT_obj_eta)
    eta2, HLT_obj_eta = ak.unzip(ak.cartesian([dataframe.Mu02_Eta, HLT_obj_eta]))
    deta2 = abs(eta2 - HLT_obj_eta)
    dphi1 = ak.where(dphi1>math.pi, dphi1 - 2*math.pi, dphi1)
    dphi2 = ak.where(dphi2>math.pi, dphi2 - 2*math.pi, dphi2)
    #print(dphi1)
    ...
    index_pt1_sel = ak.where(ak.num(index_pt1_sel, axis =1)==0, ak.Array([[-1]]), index_pt1_sel)
    pt_index1 = ak.local_index(dataframe.Mu01_Pt, axis = 1)
    ...
    trigger_support, _ = ak.unzip(ak.cartesian([pt_index1, index_pt1_sel]))
    #print(trigger_support[dataframe.evt == 2313273593])
    trigger_support = ak.run_lengths(trigger_support)
    #print(trigger_support[dataframe.evt == 2313273593])
    ...
    trigger_match1_pairs = ak.unflatten(ak.cartesian([pt_index1, index_pt1_sel]), ak.flatten(trigger_support), axis = 1)
    print(trigger_match1_pairs[dataframe.evt == 2313273593])
    #print(trigger_match1_pairs[ak.num(ak.cartesian([pt_index1, index_pt1[dRdpt_cut1]]), axis = 1)>0])
    trigger_match1 = ak.any(trigger_match1_pairs["0"]==trigger_match1_pairs["1"], axis = 2)
    print(trigger_match1[dataframe.evt == 2313273593])
    #print(pt_index1)
    #print(trigger_match1[ak.num(ak.cartesian([pt_index1, index_pt1[dRdpt_cut1]]), axis = 1)>0])
    #print(dR_cut1)
    #triggerMatch_1 = ak.values_astype(ak.any(dpt_cut1 & dR_cut1, axis = 1), int)
    dR2 = np.sqrt(dphi2*dphi2 + deta2*deta2)
    dR_cut2 = (dR2<0.03)
    dpt_cut2 = (dpt2<0.1)
    dRdpt_cut2 = dpt_cut2 & dR_cut2
    index_pt2_sel = index_pt2[dRdpt_cut2]
    index_pt2_sel = ak.where(ak.num(index_pt2_sel, axis =1)==0, ak.Array([[-1]]), index_pt2_sel)
    pt_index2 = ak.local_index(dataframe.Mu02_Pt, axis = 1)
    # Apply the function
    #trigger_match2 = contains_elements(pt_index2, index_pt2[dRdpt_cut2])
    trigger_support2, _ = ak.unzip(ak.cartesian([pt_index2, index_pt2_sel]))
    trigger_support2 = ak.run_lengths(trigger_support2)
    #print(trigger_support[ak.num(ak.cartesian([pt_index1, index_pt1[dRdpt_cut1]]), axis = 1)>0])
    #trigger_support = ak.where(ak.num(trigger_support, axis =1) == 0, ak.Array([[1]]), trigger_support)
    trigger_match2_pairs = ak.unflatten(ak.cartesian([pt_index2, index_pt2_sel]), ak.flatten(trigger_support2), axis = 1)
    #print(trigger_match1_pairs[ak.num(ak.cartesian([pt_index1, index_pt1[dRdpt_cut1]]), axis = 1)>0])
    trigger_match2 = ak.any(trigger_match2_pairs["0"]==trigger_match2_pairs["1"], axis = 2)
    #print(pt_index1)
    print(trigger_match2)
    #print(dR_cut1)
...
def process_event(sub_df, debugMode, cutevt, df_fin, isMC, tree_Filled):
    start1 = time.time()
    jentry = np.arange(ak.num(sub_df["run"], axis = 0)) #array of indeces
    print(jentry)
    ...
    evt_n = sub_df.evt
    print("NO CUTS:")
    print(ak.num(ak.drop_none(sub_df.MuonPt[ak.num(sub_df.MuonPt, axis = 1)>0]), axis = 0))  #in this way because the negation as logic condition is a bit problematic
    ...
    #cutevt[0] = ak.num(sub_df[NoGoodTriplets], axis = 0)  #is the number of events that do not have good triplets
    #cutevt[0] = ak.num(sub_df[GoodTriplets], axis = 0)  #is the number of events that have good triplets
    #print(cutevt[0])
    sub_df = sub_df[ak.any(GoodTriplets, axis = 1)]
    # CUT 1 : Check L1 and HLT decision
    # Check L1trigger
    #print(sub_df.Trigger_l1decision)
    #print((ak.str.match_substring(sub_df.Trigger_l1name,"L1_DoubleMu0er1p4_SQ_OS_dR_Max1p4"))|(ak.str.match_substring(sub_df.Trigger_l1name,"L1_DoubleMu0er1p5_SQ_OS_dR_Max1p4"))|(ak.str.match_substring(sub_df.Trigger_l1name,"L1_DoubleMu4_SQ_OS_dR_Max1p2"))|(ak.str.match_substring(sub_df.Trigger_l1name,"L1_DoubleMu4p5_SQ_OS_dR_Max1p2"))|(ak.str.match_substring(sub_df.Trigger_l1name,"L1_DoubleMu0er2p0_SQ_OS_dEta_Max1p6"))|(ak.str.match_substring(sub_df.Trigger_l1name,"L1_DoubleMu0er2p0_SQ_OS_dEta_Max1p5")))
    L1_passed = (ak.any(((ak.str.match_substring(sub_df.Trigger_l1name,"L1_DoubleMu0er1p4_SQ_OS_dR_Max1p4"))|(ak.str.match_substring(sub_df.Trigger_l1name,"L1_DoubleMu0er1p5_SQ_OS_dR_Max1p4"))|(ak.str.match_substring(sub_df.Trigger_l1name,"L1_DoubleMu4_SQ_OS_dR_Max1p2"))|(ak.str.match_substring(sub_df.Trigger_l1name,"L1_DoubleMu4p5_SQ_OS_dR_Max1p2"))|(ak.str.match_substring(sub_df.Trigger_l1name,"L1_DoubleMu0er2p0_SQ_OS_dEta_Max1p6"))|(ak.str.match_substring(sub_df.Trigger_l1name,"L1_DoubleMu0er2p0_SQ_OS_dEta_Max1p5")))&((sub_df.Trigger_l1decision) == 1), axis =1)) #keepdims = True
    print(L1_passed)
    #cutevt[0] += ak.num(sub_df, axis = 0) - ak.num(sub_df[L1_passed], axis = 0)  #in this way because the negation as logic condition is a bit problematic
    cutevt[0] = ak.num(ak.drop_none(sub_df.MuonPt[L1_passed][ak.num(sub_df.MuonPt[L1_passed], axis = 1)>0]), axis = 0)  #in this way because the negation as logic condition is a bit problematic
    print("L1 Trigger:")
    print(cutevt[0])
    #From now on I ony take into account the events that have at least one of the required L1 passed.
    sub_df = sub_df[L1_passed]
    L1seed = (1*ak.values_astype(ak.any(ak.str.match_substring(sub_df.Trigger_l1name,"L1_DoubleMu0er1p5_SQ_OS_dR_Max1p4") & (sub_df.Trigger_l1decision == 1), axis = 1), int)
            + 10*ak.values_astype(ak.any(ak.str.match_substring(sub_df.Trigger_l1name,"L1_DoubleMu4_SQ_OS_dR_Max1p2") & (sub_df.Trigger_l1decision == 1), axis = 1), int)
            + 1000*ak.values_astype(ak.any(ak.str.match_substring(sub_df.Trigger_l1name,"L1_DoubleMu0er2p0_SQ_OS_dEta_Max1p6") & (sub_df.Trigger_l1decision == 1), axis =1), int))
    # Check HLT
    HLT_passed = ak.any(ak.str.match_substring(sub_df.Trigger_hltname,"HLT_DoubleMu3_Trk_Tau3mu_v") & (sub_df.Trigger_hltdecision == 1), axis = 1).to_numpy()
    #cutevt[1] = ak.num(sub_df, axis = 0) - ak.num(sub_df[HLT_passed], axis = 0)
    cutevt[1] = ak.num(ak.drop_none(sub_df.MuonPt[HLT_passed][ak.num(sub_df.MuonPt[HLT_passed], axis = 1)>0]), axis=0)  #in this way because the negation as logic condition is a bit problematic
    print("HLT Trigger")
    print(cutevt[1])
    sub_df = sub_df[HLT_passed]
    HLTpath = (1*ak.values_astype(ak.any(ak.str.match_substring(sub_df.Trigger_hltname, "HLT_string_DoubleMu3_Trk_Tau3mu_v") & (sub_df.Trigger_hltdecision == 1), axis = 1), int)
            + 10*ak.values_astype(ak.any(ak.str.match_substring(sub_df.Trigger_hltname, "HLT_DoubleMu4_3_LowMass") & (sub_df.Trigger_hltdecision == 1), axis = 1), int)
            + 100*ak.values_astype(ak.any(ak.str.match_substring(sub_df.Trigger_hltname, "HLT_DoubleMu4_LowMass_Displaced") & (sub_df.Trigger_hltdecision == 1), axis = 1), int))
    ...
    #sub_df = sub_df[ak.num(sub_df["Mu01_Pt"], axis  =1)==ak.num(sub_df["Mu01_energy"], axis = 1)]
    sub_df = sub_df[ak.num(sub_df["Mu01_energy"], axis  =1)==ak.num(sub_df["Mu01_Pt"], axis = 1)]
    sub_df = sub_df[ak.num(sub_df["Mu01_charge"], axis  =1)==ak.num(sub_df["Mu02_charge"], axis = 1)]
    ...
    triggerMatch_1, triggerMatch_2, triggerMatch_3 = trigger_match(sub_df)
    print(ak.num(triggerMatch_1, axis = 1))
    print(sub_df.evt[(ak.num(sub_df.Mu01_Pt, axis = 1)!=ak.num(triggerMatch_1, axis = 1))])
    for col in ak.fields(sub_df):
        if col.startswith("Mu0") or col.startswith("Tr_"):  # Pattern matching, adjust as necessary
            sub_df[col] = sub_df[col][acceptance&not_duplicated&cut_Pv_Sv_significance&muons_GlbMdm]#&triMuonMass]
    events_now = sub_df["evt"][ak.num(sub_df.Mu01_Pt, axis = 1)>0]
    sub_df["Triplet2_Mass"] = sub_df["Triplet2_Mass"][acceptance&not_duplicated&cut_Pv_Sv_significance&muons_GlbMdm]
    print("MUON ID:")
    #print(ak.sum(ak.flatten(muons_GlbMdm, axis=0), axis = 0))
    print(ak.num(sub_df.Mu01_Pt[ak.num(sub_df.Mu01_Pt, axis = 1)>0], axis = 0))
    sub_df["Mu0102_Mass"] = DimuonMass(sub_df)
    ...
    print(ak.num(sub_df.Mu01_Pt[ak.num(sub_df.Mu01_Pt, axis = 1)>0], axis = 0))
    sub_df = sub_df[ak.num(sub_df.Mu01_Pt, axis = 1)>0]
    new_df = {}
    for col in ak.fields(sub_df):
        if col.startswith("Mu0") or col.startswith("Tr_"):# or col.startswith("TripletVtx2_Chi2"):  # Pattern matching, adjust as necessary
            new_df[col] = ak.flatten(sub_df[col][sub_df.TripletVtx2_Chi2 == ak.min(sub_df.TripletVtx2_Chi2, axis = 1)])
    print(sub_df)
    stop1 = time.time()
    dtime1 = stop1 -start1
...
def Loop_DsPhiPi(tree, type, datasetName, filename):
    ...
    arrays = process_event(df, debugMode, cutevt, df_fin, isMC, tree_Filled)
    #print(arrays)
    #arrays = ak.to_rdataframe(arrays)
