...
void myAnalizer_control::Loop_DsPhiPi(TString type, TString datasetName)
{
    ...
    int cutevt[NCUTS] = {0};
    uint goodMuonID_evt[569] = {0};
    uint goodMuonID_run[569] = {0};
    int id = 0;
    ...
    for (Long64_t jentry=0; jentry<nentries;jentry++) {
        ...
        if(evt == 2447296188){ debugMode = true;}
        ...
        for (int j=0; j<TripletVtx2_Chi2->size(); j++){
            ...
            if(debugMode){
            cout << Muon_isGlobal->at(mu[0]) << "," << Muon_isMedium->at(mu[0]) << ","<< Muon_isGlobal->at(mu[1]) << "," << Muon_isMedium->at(mu[1]) << endl;
            cout << Mu01_Pt->at(j) << "," << Mu02_Pt->at(j) << "," << Mu01_Eta->at(j) << "," << Mu02_Eta->at(j) << "," << Tr_Pt->at(j) << "," << Tr_Eta->at(j) << endl;
            cout << Track_dz->at(mu[2]) << "," << Track_dxy->at(mu[2])<< endl;
            }
            ...
            //if(Muon_isMedium->at(mu[0]) == 1 && Muon_isMedium->at(mu[1]) == 1 && Mu01_Pt->at(j) >= 2 && Mu02_Pt->at(j) >= 2 && abs(Mu01_Eta->at(j))<=2.4 && abs(Mu02_Eta->at(j))<=2.4 && Tr_Pt->at(j) >= 2 && abs(Tr_Eta->at(j))<=2.4 && (Track_dz->at(mu[2]) < 20 && Track_dxy->at(mu[2]) <0.3)) { // check duplicates
                ...
                if(debugMode){
                    cout << "is Mdm&Glb" << endl;
                    if(isDupl){cout << "not duplicated" << endl;}
                    if(FlightDistBS_SV_Significance->at(j) < 3.5){cout << FlightDistBS_SV_Significance->at(j) << endl;}}
                if(isDupl && FlightDistBS_SV_Significance->at(j) >= 3.5 ){ good_muonID = true; goodMuonID_evt[id] = evt_n; goodMuonID_run[id] = run_n; id++; if(debugMode) cout << "goodMuonID" << endl;}
            }
            if(!good_muonID){
                continue;}
    ...
    std::string filename = "events_surviving_goodMuonId.csv";

    // Open an output file stream in write mode
    ...
    // Optional: Add a newline after the values (for a single-row CSV)
    file << "\n";
