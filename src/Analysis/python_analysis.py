...
def goodTriplet_selection(sub_df_selection, sub_df, debugMode, triplEff_counter, HLT_obj_phi, HLT_obj_eta, HLT_obj_pt,  jentry, df_fin, isMC, ind_good, mu_Ind_good, mu_good, lumi_n, run_n, evt_n, L1_DoubleMu0_er1p5, L1_DoubleMu0_er1p4, L1_DoubleMu4_dR1p2,L1_DoubleMu4p5_dR1p2, L1_DoubleMu0_er2p0, L1_DoubleMu0_er2p0_bk, L1seed, HLTpath, cutevt):
    ...
    not_duplicate = DuplicateFinder(sub_df, eta1, eta2, etaTr, phi1, phi2, phiTr, pt1, pt2, ptTr) and FlightDistBS_SV_Significance>=3.5
    ...
    if (DiMuMass<=1.06 and DiMuMass>=0.98): 
        ...
        sub_df_selection["good_diMuMass"] = False
        cutevt[4]+=1
        return None
    if (Triplet2_Mass<=2.1 and Triplet2_Mass>=1.62): 
        ...
        sub_df_selection["good_triMuMass"] = False
        cutevt[5]+=1
        return None
    #print(HLT_obj_phi)
...
def process_event(sub_df, debugMode, cutevt, df_fin, isMC, tree_Filled):
    ...
    print(sub_df.Trigger_l1decision.iloc[0] == 1)
    ...
    if HLT_passed:
        ...
        print(f"HLT_path (after) = {HLTpath}")
        triplEff_counter[1] = True
    else:
        for i in range(2):
    ...
    if 'mu1_TrMatch' not in triplet_df.columns and 'mu12_TrMatch' not in triplet_df.columns and 'mu123_TrMatch' not in triplet_df.columns:
        return None
    triplet_df = triplet_df[(triplet_df.mu1_TrMatch == True) & (triplet_df.mu12_TrMatch == True) & (triplet_df.mu123_TrMatch == True)]
    if len(triplet_df.number_of_triplet.unique())>=1:
        #print(f"more than one good triplet {triplet_df.number_of_triplet.unique()}")
        #print(sub_df)
        if len(triplet_df.number_of_triplet.unique())>1:ind_good = BestTripletFinder(sub_df, triplet_df.number_of_triplet.unique())
        else: ind_good = triplet_df.number_of_triplet.unique()[0]
        print(ind_good)
...
def Loop_DsPhiPi(tree, type, datasetName, filename):
    ...
    tree_Filled = 0
    df.groupby(df.index).apply(process_event, debugMode, cutevt, df_fin, isMC, tree_Filled)
