...
void myAnalizer::Loop_Tau3mu(TString type, TString datasetName)
{
    ...
    if(datasetName.Contains("2022") || datasetName.Contains("2023")) isMC=0;
        ...
        if(datasetName.Contains("Run4")) isMC = 4;
    }
    TH1D *histo_muonEta = new TH1D("MuonEta", "MuonEta", 500, 0, 3);
    histo_muonEta->SetXTitle("|Eta|");
    TH1D *histo_PfMuonEta = new TH1D("PfMuonEta", "MuonEta", 500, 0, 3);
    histo_muonEta->SetXTitle("|Eta|");
    TH2F *h2 = new TH2F("h2", "", 
                        500, 0, 2.8,  
                        300, 0, 30);
    h2->SetXTitle("|Eta|");
    h2->SetYTitle("Pt");
    TH1D *histo_Me0MuonEta = new TH1D("Me0MuonEta", "MuonEta", 500, 0, 3);
    histo_Me0MuonEta->SetXTitle("|Eta|");
    TH1D *histo_TrackerMuonEta = new TH1D("TrackerMuonEta", "MuonEta", 500, 0, 3);
    histo_TrackerMuonEta->SetXTitle("|Eta|");
    TH1D *histo_TrackerOutMuonEta = new TH1D("TrackerOutMuonEta", "MuonEta", 500, 0, 3);
    histo_TrackerOutMuonEta->SetXTitle("|Eta|");
    TH1D *histo_GEMMuonEta = new TH1D("GEMMuonEta", "MuonEta", 500, 0, 3);
    histo_GEMMuonEta->SetXTitle("|Eta|");
    TH2F *h2Me0 = new TH2F("h2Me0", "", 
                        500, 0, 3,  
                        300, 0, 30);
    h2Me0->SetXTitle("|Eta|");
    h2Me0->SetYTitle("Pt");
    for (Long64_t jentry=0; jentry<nentries;jentry++) {
        ...
        for(int k=0; k<MuonEta->size();k++){
            histo_muonEta -> Fill(std::abs(MuonEta->at(k)));
            h2 -> Fill(std::abs(MuonEta->at(k)), MuonPt->at(k));
            if (Muon_isME0->at(k)==1){
                histo_Me0MuonEta -> Fill(std::abs(MuonEta->at(k)));
            }
            if (Muon_isPF->at(k)==1){
                histo_PfMuonEta -> Fill(std::abs(MuonEta->at(k)));
            }
            if (Muon_isGEM->at(k)==1){
                histo_GEMMuonEta -> Fill(std::abs(MuonEta->at(k)));
            }
            if (Muon_isTrackerMuon->at(k)==1){
                histo_TrackerOutMuonEta -> Fill(std::abs(Muon_outerTrack_eta->at(k)));
                histo_TrackerMuonEta -> Fill(std::abs(MuonEta->at(k)));
            }
        }
        histo_muonEta->SetLineColor(kRed);
        histo_Me0MuonEta->SetLineColor(kBlue);
        histo_TrackerOutMuonEta->SetLineColor(kViolet);
        histo_PfMuonEta->SetLineColor(kGreen);
        histo_TrackerMuonEta->SetLineColor(kBlack);
        ...
        for (int j=0; j<TripletVtx_Chi2->size(); j++){
            ...
            ////if(Muon_isGlobal->at(mu[0]) == 1 && Muon_isMedium->at(mu[0]) == 1 && Muon_isGlobal->at(mu[1]) == 1 && Muon_isMedium->at(mu[1]) == 1 && Muon_isGlobal->at(mu[2]) == 1 && Muon_isMedium->at(mu[2]) == 1 && Mu1_Pt->at(j)>=1.5 && Mu2_Pt->at(j)>=1.5  && Mu3_Pt->at(j)>=1.5 && abs(Mu1_Eta->at(j))<=2.8 && abs(Mu2_Eta->at(j))<=2.8 && abs(Mu3_Eta->at(j))<=2.8){
            if(Mu1_Pt->at(j)>=1.5 && Mu2_Pt->at(j)>=1.5  && Mu3_Pt->at(j)>=1.5 && abs(Mu1_Eta->at(j))<=2.8 && abs(Mu2_Eta->at(j))<=2.8 && abs(Mu3_Eta->at(j))<=2.8){
            ...
            if(Triplet_Mass->at(j)<=2.5 && Triplet_Mass->at(j)>=1.){
    ...
    TCanvas *canvEta = new TCanvas("MuonEta", "MuonEta", 0, 0, 1200, 1000);
    TLegend* legend = new TLegend(0.1, 0.7, 0.3, 0.9);
    legend->AddEntry(histo_muonEta, "All Muons", "l");
    legend->AddEntry(histo_Me0MuonEta, "ME0 Muons", "l");
    legend->AddEntry(histo_GEMMuonEta, "GEM Muons", "l");
    legend->AddEntry(histo_PfMuonEta, "PF Muons", "l");
    legend->AddEntry(histo_TrackerMuonEta, "Tracker Muons", "l");
    legend->AddEntry(histo_TrackerOutMuonEta, "Outer Tracker Muons", "l");
    histo_muonEta -> Draw();
    histo_Me0MuonEta -> Draw("same");
    histo_GEMMuonEta -> Draw("same");
    histo_PfMuonEta -> Draw("same");
    histo_TrackerMuonEta -> Draw("same");
    histo_TrackerOutMuonEta -> Draw("same");
    legend -> Draw("same");
    TString histosave = fileName + ".png";
    canvEta -> SaveAs(histosave);
    TCanvas *canvEtaPt = new TCanvas("MuonPtEta", "MuonPtEta", 0, 0, 1200, 1000);
    h2 -> Draw();
    //h2Me0 -> Draw();
    TString histosave1 = fileName + "_histo2d.png";
    canvEtaPt -> SaveAs(histosave1);
